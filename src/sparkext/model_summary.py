...
class ModelSummary(ABC):
    ...
    def signature(self) -> ModelSignature:
        """Get MLFlow model signature"""
        return ModelSignature(inputs=Schema(self.inputs),
                              outputs=Schema(self.outputs))

    def get_return_type(self, names=False) -> str:
        """Get Spark SQL type string for output of the model."""

        def type_str(tensor_spec: TensorSpec) -> str:
            # map tensor type to spark sql type str
            udf_type = udf_types[tensor_spec.type]
            name = tensor_spec.name
            # wrap sql type str with 'array', if needed
            tensor_type = f"array<{udf_type}>" if len(tensor_spec.shape) > 0 else udf_type
